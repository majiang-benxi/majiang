...
<div class="pd-20">
	<div class="text-c"> 
	用户ID：<input type="text" class="input-text" style="width:250px" placeholder="房用户ID" id="userid" name=""/>
	<span class="select-box inline">
	变化类型：
	<select name="" class="select" id="changeType">
...
var changeItemPage = function(curPageP){
	...
	if(changeType==null || changeType=='' || changeType=='undefined'){
	 ...
	 $.ajax({
			type: "get",
			url : "/mvc/statistics/getRoomCardCahngeInfoList?userid="+userid+"&changeType="+changeType+"&datemin="+datemin+"&datemax="+datemax+"&eachPageCount="+eachPageCount+"&curPage="+curPage+"&t="+new Date().getTime(),
			dataType:'html',
			data: '', 
			async:true,
