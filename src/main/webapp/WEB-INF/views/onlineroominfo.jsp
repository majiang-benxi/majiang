...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 实时管理 <span class="c-gray en">&gt;</span> 在线房间 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" id="frontuserRef" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<div class="text-c"> 
...
var changeItemPage = function(curPageP){
	...
	if(roomnum==null || roomnum=='' || roomnum=='undefined'){
	 ...
	 $.ajax({
			type: "get",
			url : "/mvc/statistics/getRoomOnLineList?roomnum="+roomnum+"&datemin="+datemin+"&datemax="+datemax+"&eachPageCount="+eachPageCount+"&curPage="+curPage+"&t="+new Date().getTime(),
			dataType:'html',
			data: '', 
			async:true,
...
/*管理员-启用*/
function killRoom(obj,id){
	layer.confirm('确认要解散吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
			
		$.ajax({
		    url:'/mvc/statistics/killRoom',
		    type:'POST', //GET
		...
		$(obj).remove();
		layer.msg('已解散!', {icon: 6,time:1000});
	});
}
