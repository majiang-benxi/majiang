			...
			if (protocolModel.getBody() == null) {
				...
				try {
					if (discardReqModel.getTileGroupType() == TileGroupType.ONE_GROUP.getCode()) {
						...
						List<ActionAndLocation> actionAndLocations = HandlerHelper
					...
					} else if (discardReqModel.getTileGroupType() == TileGroupType.XUAN_FENG_GANG_ZFB_GROUP.getCode()) {
						HandlerHelper.xfgProcess2Players(roomContext, XUAN_FENG_GANG_ZFB_GROUP,
								new Action(ZIPAI, discardReqModel.getTile()), discardPlayLocation);
					} else if (discardReqModel.getTileGroupType() == TileGroupType.XUAN_FENG_GANG_DNXB_GROUP
							.getCode()) {
						...
						List<ActionAndLocation> needPassOrDoActions = roomContext.getGameContext()
